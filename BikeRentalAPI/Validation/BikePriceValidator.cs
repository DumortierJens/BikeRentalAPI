...
public class UpdateBikePriceValidation : AbstractValidator<BikePrice>
{
    public UpdateBikePriceValidation()
    {
        RuleFor(x => x.Id).Matches(@"^[0-9a-f]{24}$").WithMessage("Id has not a valid 24 digit hex string");
        RuleFor(x => x.Prices).NotEmpty().SetValidator(new PricesValidation());
    }
}